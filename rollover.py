"""
Refreshes data/active_markets.txt with today's most liquid daily-high-temp
bucket market per city. Run once daily (before today's markets have had time
to expire) — see com.kalshimm.rollover.plist, which forces a bot restart
right after this runs so main.py picks up the fresh ticker list.
...
def main() -> None:
    ...
    tickers = market_discovery.discover_active_markets(date.today())
